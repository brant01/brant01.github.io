...
##Treeatment by facility

###Number of cases by facility
```{r}
data <- data %>%
        add_count(PUF_FACILITY_ID)
        

data %>%
        select(PUF_FACILITY_ID, n) %>%
        unique() %>%
        ggplot(aes(x = n)) +
...
fit_surg_h <- glm(SURG_TX ~ AGE + SEX + CDCC_TOTAL + T_SIZE + FACILITY_TYPE_F + PUF_FACILITY_ID +
                        YEAR_OF_DIAGNOSIS,
                family = "binomial",
                data = data)
...
```{r}
exp(cbind("Odds ratio" = coef(fit_surg_h), confint.default(fit_surg_h, level = 0.95)))
```

```{r}
OR_table <- as.data.frame(exp(cbind("Odds ratio" = coef(fit_surg_h), confint.default(fit_surg_h, level = 0.95)))) %>%
        rownames_to_column() %>%
        mutate(fac_id = str_detect(rowname, "PUF_FACILITY")) %>%
        filter(fac_id == TRUE) %>%
        rename(OR = `Odds ratio`) %>%
        filter(is.na(OR) == FALSE)
