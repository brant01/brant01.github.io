...
data <- data %>% mutate(FACILITY_LOCATION_F = fct_relevel(YEAR_OF_DIAGNOSIS,
                                             "2014", after = 14))
...
p_table(data,
        vars = c("FACILITY_TYPE_F", "FACILITY_LOCATION_F", "FACILITY_GEOGRAPHY",  "AGE", "AGE_F", "AGE_40",
                 ...
                 "SURG_RAD_SEQ_C", "T_SIZE", "SURGERY_YN", "RADIATION_YN", "CHEMO_YN", "mets_at_dx", "LATERALITY"))
...
data %>%
    ggplot(aes(x = YEAR_OF_DIAGNOSIS)) + 
    geom_histogram(stat = "count", aes(fill = HISPANIC), position = "fill")
```
...
data %>%
    ggplot(aes(x = YEAR_OF_DIAGNOSIS)) + 
    geom_histogram(stat = "count", aes(fill = AGE_F), position = "fill")
```
...
data %>%
    ggplot(aes(x = YEAR_OF_DIAGNOSIS)) + 
    geom_histogram(stat = "count", aes(fill = RACE_F), position = "fill")
```
...
```{r}
p_table(data,
        vars = c("AGE_F", "RACE_F", "SEX_F", "INCOME_F", "EDUCATION_F", "INSURANCE_F", "U_R_F"),
        strata = c("FACILITY_LOCATION_F"))
```
...
#Tumor size by year
Limited to below 10 cm to exclude recording errors

```{r}
data <- data %>%
        mutate(TUMOR_SIZE_LIM = ifelse(TUMOR_SIZE < 100,
                                       TUMOR_SIZE,
                                        NA)) %>%
        mutate(T_SIZE_LIM = fct_recode(T_SIZE,
                                       NULL = "NA_unk"))

p_table(data,
        vars = c("TUMOR_SIZE", "TUMOR_SIZE_LIM", "T_SIZE_LIM"),
        strata = c("YEAR_OF_DIAGNOSIS"))

data %>%
        mutate(YEAR = as.numeric(as.character(YEAR_OF_DIAGNOSIS))) %>%
        lm(TUMOR_SIZE_LIM ~ YEAR,
              data = .) %>%
        summary()

data %>%
    ggplot(aes(x = TUMOR_SIZE_LIM)) +
    geom_histogram() +
    facet_wrap(~ YEAR_OF_DIAGNOSIS)

data %>%
    ggplot(aes(x = YEAR_OF_DIAGNOSIS, y = TUMOR_SIZE_LIM)) + 
    geom_boxplot() 
...
model_one <- coxph(Surv(DX_LASTCONTACT_DEATH_MONTHS, PUF_VITAL_STATUS == 0)
                     ~ SURG_RAD_SEQ + INSURANCE_F + AGE + SEX_F + RACE_F + INCOME_F + U_R_F + CDCC_TOTAL +
                      FACILITY_TYPE_F + FACILITY_LOCATION_F + EDUCATION_F + T_SIZE,
                     data = data)
model_one %>% summary()
