---
title: "NCDB Sinonasal Squamous Cell Carcinoma 2015"
author: "Jason Brant"
...
bone.joint <- read_fwf('NCDB_PUF_DATA_Nov-07-2017/NCDBPUF_BoneJont.3.2015.0.dat', 
                       fwf_widths(col.width, col_names = col.names.abr),
                       col_types = cols(.default = col_character()))

brain <- read_fwf('NCDB_PUF_DATA_Nov-07-2017/NCDBPUF_Brain.3.2015.0.dat', 
                       fwf_widths(col.width, col_names = col.names.abr),
                       col_types = cols(.default = col_character()))

cns <- read_fwf('NCDB_PUF_DATA_Nov-07-2017/NCDBPUF_CNS.3.2015.0.dat', 
                       fwf_widths(col.width, col_names = col.names.abr),
                       col_types = cols(.default = col_character()))

gum.ot.mth <- read_fwf('NCDB_PUF_DATA_Nov-07-2017/NCDBPUF_GumOtMth.3.2015.0.dat', 
                       fwf_widths(col.width, col_names = col.names.abr),
                       col_types = cols(.default = col_character()))

hypophar <- read_fwf('NCDB_PUF_DATA_Nov-07-2017/NCDBPUF_Hypophar.3.2015.0.dat', 
                       fwf_widths(col.width, col_names = col.names.abr),
                       col_types = cols(.default = col_character()))

larynx <- read_fwf('NCDB_PUF_DATA_Nov-07-2017/NCDBPUF_Larynx.3.2015.0.dat', 
                       fwf_widths(col.width, col_names = col.names.abr),
                       col_types = cols(.default = col_character()))

lip <- read_fwf('NCDB_PUF_DATA_Nov-07-2017/NCDBPUF_Lip.3.2015.0.dat', 
                       fwf_widths(col.width, col_names = col.names.abr),
                       col_types = cols(.default = col_character()))

melanoma <- read_fwf('NCDB_PUF_DATA_Nov-07-2017/NCDBPUF_Melanoma.3.2015.0.dat', 
                       fwf_widths(col.width, col_names = col.names.abr),
                       col_types = cols(.default = col_character()))

fom <- read_fwf('NCDB_PUF_DATA_Nov-07-2017/NCDBPUF_MouthFlr.3.2015.0.dat', 
                       fwf_widths(col.width, col_names = col.names.abr),
                       col_types = cols(.default = col_character()))

nasal <- read_fwf('NCDB_PUF_DATA_Nov-07-2017/NCDBPUF_Nasal.3.2015.0.dat', 
                       fwf_widths(col.width, col_names = col.names.abr),
                       col_types = cols(.default = col_character()))

nasophar <- read_fwf('NCDB_PUF_DATA_Nov-07-2017/NCDBPUF_Nasophar.3.2015.0.dat', 
                       fwf_widths(col.width, col_names = col.names.abr),
                       col_types = cols(.default = col_character()))

orophar <- read_fwf('NCDB_PUF_DATA_Nov-07-2017/NCDBPUF_Orophary.3.2015.0.dat', 
                       fwf_widths(col.width, col_names = col.names.abr),
                       col_types = cols(.default = col_character()))

oth.endo <- read_fwf('NCDB_PUF_DATA_Nov-07-2017/NCDBPUF_OtEndocr.3.2015.0.dat', 
                       fwf_widths(col.width, col_names = col.names.abr),
                       col_types = cols(.default = col_character()))

skin <- read_fwf('NCDB_PUF_DATA_Nov-07-2017/NCDBPUF_OtSkin.3.2015.0.dat', 
                       fwf_widths(col.width, col_names = col.names.abr),
                       col_types = cols(.default = col_character()))

pharynx <- read_fwf('NCDB_PUF_DATA_Nov-07-2017/NCDBPUF_Pharynx.3.2015.0.dat', 
                       fwf_widths(col.width, col_names = col.names.abr),
                       col_types = cols(.default = col_character()))

saliv.gland <- read_fwf('NCDB_PUF_DATA_Nov-07-2017/NCDBPUF_SalivGld.3.2015.0.dat', 
                       fwf_widths(col.width, col_names = col.names.abr),
                       col_types = cols(.default = col_character()))

thyroid <- read_fwf('NCDB_PUF_DATA_Nov-07-2017/NCDBPUF_Thyroid.3.2015.0.dat', 
                       fwf_widths(col.width, col_names = col.names.abr),
                       col_types = cols(.default = col_character()))

tongue <- read_fwf('NCDB_PUF_DATA_Nov-07-2017/NCDBPUF_Tongue.3.2015.0.dat', 
                       fwf_widths(col.width, col_names = col.names.abr),
                       col_types = cols(.default = col_character()))

tonsil <- read_fwf('NCDB_PUF_DATA_Nov-07-2017/NCDBPUF_Tonsil.3.2015.0.dat', 
                       fwf_widths(col.width, col_names = col.names.abr),
                       col_types = cols(.default = col_character()))

trach.med <- read_fwf('NCDB_PUF_DATA_Nov-07-2017/NCDBPUF_TrachMed.3.2015.0.dat', 
                       fwf_widths(col.width, col_names = col.names.abr),
                       col_types = cols(.default = col_character()))
...
p_table(data,
        vars = c("FACILITY_TYPE_F", "FACILITY_LOCATION_F", "FACILITY_GEOGRAPHY",  "AGE", "AGE_F", "AGE_40",
                 ...
                 "SURG_RAD_SEQ_C", "T_SIZE", "SURGERY_YN", "RADIATION_YN", "CHEMO_YN", "mets_at_dx", "CS_METS_AT_DX",
...
noUknHormones <- data %>% filter(RX_HOSP_HORMONE != 99)




data %>% 
  mutate(SURG_RAD_SEQ = fct_collapse(SURG_RAD_SEQ, "No Treatment" = "No Treatement",
                                   "Surgery and Radiation" = c("Surg then Rad", 
                                                               "Rad before and after Surg",
                                                               "Rad then Surg"),
                                   "Radiation Alone" = "Rad Alone",
                                   "Surgery Alone" = "Surg Alone")) %>%
    ggplot(aes(x = YEAR_OF_DIAGNOSIS)) + 
    geom_histogram(stat = "count", aes (fill = SURG_RAD_SEQ), position = "fill") +
  labs(title = "Distribution of Treatment Modality By Year for Pituitary Adenoma",
       x = "Year of Diagnosis", 
...
data <- data %>% mutate(TX = !is.na(DX_RX_STARTED_DAYS))

data$TX <- as.factor(data$TX)

levels(data$TX) <- c("No Treatment/Watchful Waiting", "Treated")
...
data %>% 
    ggplot(aes(x = YEAR_OF_DIAGNOSIS)) + 
    geom_histogram(stat = "count", aes (fill = TX), position = "fill") +
  labs(title = "Proportion of Patients who Received 
       Treatment for Diagnosed Pituitary Adenoma by Year",
       x = "Year of Diagnosis", 
       y = "Proportion of Total Patients (%)",
...
data2 <- data %>% 
  filter(RX_SUMM_HORMONE != 99) %>%
  mutate(TX2 = SURG_RAD_SEQ != "No Treatement" & (RX_SUMM_HORMONE != 00))

data2$TX2 <- as.factor(data2$TX2)

levels(data2$TX2) <- c("No Treatment/Watchful Waiting", "Treated")
...
data2 %>% 
    ggplot(aes(x = YEAR_OF_DIAGNOSIS)) + 
    geom_histogram(stat = "count", aes (fill = TX2), position = "fill") +
  labs(title = "Proportion of Patients who Received 
       Treatment for Diagnosed Pituitary Adenoma by Year",
       x = "Year of Diagnosis", 
       y = "Proportion of Total Patients (%)",
...
noUkns <-  data %>% filter(T_SIZE != "NA_unk")
...
```{r}
no_Ukns <- data %>% filter(RADIATION_YN != "Ukn")

p_table(no_Ukns,
        vars = c("RADIATION_YN"),
        strata = c("YEAR_OF_DIAGNOSIS"))

micros <- data %>% filter(T_SIZE == "< 1 cm")
...
data %>%
    ...
    geom_histogram(stat = "count", aes (fill = HISTOLOGY), position = "fill")

data %>%
    ggplot(aes(x = AGE_F)) + 
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    geom_histogram(stat = "count", aes (fill = SEX_F), position = "fill")

#compare average ages between males and females
t.test((data %>% filter(SEX_F == "Female" & AGE != "NA"))$AGE, 
       (data%>%filter(SEX_F == "Male" & AGE != "NA"))$AGE)

p_table(no_Ukns,
        vars = c("T_SIZE"),
        strata = c("RADIATION_YN"))

no_Ukns %>%
    ggplot(aes(x = T_SIZE, fill = RADIATION_YN)) +
    geom_bar(position = "fill") +   facet_wrap(~ YEAR_OF_DIAGNOSIS)

data %>%
    ggplot(aes(x = T_SIZE, fill = SURG_RAD_SEQ)) +
    geom_bar(position = "fill") +   facet_wrap(~ YEAR_OF_DIAGNOSIS)

data %>%
    ggplot(aes(x = T_SIZE, fill = SURGERY_YN)) +
    geom_bar(position = "fill") +   facet_wrap(~ YEAR_OF_DIAGNOSIS)


# gender differences in type of PA?
data %>%
    ggplot(aes(x = HISTOLOGY, fill = AGE_F)) +
    geom_bar(position = "fill") +   facet_wrap(~ SEX_F, ncol = 1)
...
```{r}
p_table(data,
        vars = c("FACILITY_TYPE_F"),
        strata = c("RADIATION_YN"))



data %>%
    ggplot(aes(x = FACILITY_TYPE_F)) + 
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    geom_histogram(stat = "count", aes (fill = RADIATION_YN), position = "fill")
```

##Radiation by Variable, without Unknowns
```{r}
...
no_Ukns %>%
    filter(CROWFLY < 2000) %>%
    ggplot(aes(x = CROWFLY)) +
    geom_histogram() +
    facet_wrap(~ FACILITY_TYPE_F)

no_Ukns %>%
    filter(CROWFLY < 2000) %>%
    ggplot(aes(x = CROWFLY)) +
    geom_histogram() +
    facet_wrap(~ RADIATION_YN)

# proportion of patients who traveled a certain distance who fall into 
  # different income brackets
no_Ukns %>%
    filter(CROWFLY < 1000) %>%
    ggplot(aes(x = CROWFLY, fill = INCOME_F)) +
...
data %>%
    filter(CROWFLY < 1000) %>%
    ggplot(aes(x = CROWFLY, y = DX_LASTCONTACT_DEATH_MONTHS, color = RADIATION_YN)) +
    geom_point() + 
  theme_minimal()
...
##Variable for type of radiation

```{r}
data <- data %>%
        mutate(RAD_TYPE = case_when(RAD_REGIONAL_RX_MODALITY == "00" ~ "No Radiation",
                                    RAD_REGIONAL_RX_MODALITY == "20" ~ "External Beam, NOS",
                                    RAD_REGIONAL_RX_MODALITY == "21" ~ "Orthovoltage",
                                    RAD_REGIONAL_RX_MODALITY == "22" ~ "Cobalt-60, Cesium-137",
                                    RAD_REGIONAL_RX_MODALITY == "23" ~ "Photons (2-5 MV)",
                                    RAD_REGIONAL_RX_MODALITY == "24" ~ "Photons (6-10 MV)",
                                    RAD_REGIONAL_RX_MODALITY == "25" ~ "Photons (11-19 MV)",
                                    RAD_REGIONAL_RX_MODALITY == "26" ~ "Photons (>19 MV)",
                                    RAD_REGIONAL_RX_MODALITY == "27" ~ "Photons (mixed energies)",
                                    RAD_REGIONAL_RX_MODALITY == "28" ~ "Electrons",
                                    RAD_REGIONAL_RX_MODALITY == "29" ~ "Photons and electrons mixed",
                                    RAD_REGIONAL_RX_MODALITY == "30" ~ "Neutron +/- Photons/Electrons",
                                    RAD_REGIONAL_RX_MODALITY == "31" ~ "IMRT",
                                    RAD_REGIONAL_RX_MODALITY == "32" ~ "Conformal or 3-D therapy",
                                    RAD_REGIONAL_RX_MODALITY == "40" ~ "Protons",
                                    RAD_REGIONAL_RX_MODALITY == "41" ~ "Stereotactic Surgery, NOS",
                                    RAD_REGIONAL_RX_MODALITY == "42" ~ "Linac",
                                    RAD_REGIONAL_RX_MODALITY == "43" ~ "Gamma Knife",
                                    RAD_REGIONAL_RX_MODALITY == "50" ~ "Brachytherapy, NOS",
                                    RAD_REGIONAL_RX_MODALITY == "51" ~ "Brachytherapy, Intracavitary, LDR",
                                    RAD_REGIONAL_RX_MODALITY == "52" ~ "Brachytherapy, Intracavitary, HDR",
                                    RAD_REGIONAL_RX_MODALITY == "53" ~ "Brachytherapy, Interstitial, LDR",
                                    RAD_REGIONAL_RX_MODALITY == "54" ~ "Brachytherapy, Interstitial, HDR",
                                    RAD_REGIONAL_RX_MODALITY == "55" ~ "Radium",
                                    RAD_REGIONAL_RX_MODALITY == "60" ~ "Radioisotopes, NOS",
                                    RAD_REGIONAL_RX_MODALITY == "61" ~ "Strontium-89",
                                    RAD_REGIONAL_RX_MODALITY == "62" ~ "Strontium-90",
                                    RAD_REGIONAL_RX_MODALITY == "80" ~ "Combination, specified",
                                    RAD_REGIONAL_RX_MODALITY == "85" ~ "Combination, NOS",
                                    RAD_REGIONAL_RX_MODALITY == "98" ~ "Other, NOS",
                                    RAD_REGIONAL_RX_MODALITY == "99" ~ "Unknown",
                                    TRUE ~ "NA/Unknown")) %>%
        mutate(RAD_DOSE = RAD_REGIONAL_DOSE_CGY)

p_table(data,
        vars = c("RAD_TYPE"))

data <- data %>% 
  filter(RAD_TYPE != "No Radiation") %>% 
  filter(RAD_TYPE != "Unknown")
...
##Variable for type of radiation excluding no radiation
...
data %>%
    ggplot(aes(x = FACILITY_TYPE_F)) + 
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    geom_histogram(stat = "count", aes (fill = RAD_TYPE), position = "fill") 

data %>%
    ggplot(aes(x = FACILITY_GEOGRAPHY)) + 
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    geom_histogram(stat = "count", aes (fill = RAD_TYPE), position = "fill") 
...
```{r combined radiation types}


data <- data %>%
        mutate(RAD_TYPE_F = case_when(RAD_REGIONAL_RX_MODALITY == "00" ~ "No Radiation",
                                    RAD_REGIONAL_RX_MODALITY == "20" ~ "External Beam",
                                    RAD_REGIONAL_RX_MODALITY == "21" ~ "Orthovoltage",
                                    RAD_REGIONAL_RX_MODALITY == "22" ~ "Cobalt-60, Cesium-137",
                                    RAD_REGIONAL_RX_MODALITY == "23" ~ "External Beam",
                                    RAD_REGIONAL_RX_MODALITY == "24" ~ "External Beam",
                                    RAD_REGIONAL_RX_MODALITY == "25" ~ "External Beam",
                                    RAD_REGIONAL_RX_MODALITY == "26" ~ "External Beam",
                                    RAD_REGIONAL_RX_MODALITY == "27" ~ "External Beam",
                                    RAD_REGIONAL_RX_MODALITY == "28" ~ "External Beam",
                                    RAD_REGIONAL_RX_MODALITY == "29" ~ "External Beam",
                                    RAD_REGIONAL_RX_MODALITY == "30" ~ "External Beam",
                                    RAD_REGIONAL_RX_MODALITY == "31" ~ "External Beam",
                                    RAD_REGIONAL_RX_MODALITY == "32" ~ "External Beam",
                                    RAD_REGIONAL_RX_MODALITY == "40" ~ "External Beam",
                                    RAD_REGIONAL_RX_MODALITY == "41" ~ "Stereotactic Surgery",
                                    RAD_REGIONAL_RX_MODALITY == "42" ~ "External Beam",
                                    RAD_REGIONAL_RX_MODALITY == "43" ~ "Stereotactic Surgery",
                                    RAD_REGIONAL_RX_MODALITY == "50" ~ "Brachytherapy, NOS",
                                    RAD_REGIONAL_RX_MODALITY == "51" ~ "Brachytherapy, Intracavitary, LDR",
                                    RAD_REGIONAL_RX_MODALITY == "52" ~ "Brachytherapy, Intracavitary, HDR",
                                    RAD_REGIONAL_RX_MODALITY == "53" ~ "Brachytherapy, Interstitial, LDR",
                                    ...
                                    RAD_REGIONAL_RX_MODALITY == "80" ~ "Combination, specified",
                                    RAD_REGIONAL_RX_MODALITY == "85" ~ "Combination, NOS",
                                    RAD_REGIONAL_RX_MODALITY == "98" ~ "Unknown",
                                    RAD_REGIONAL_RX_MODALITY == "99" ~ "Unknown",
                                    TRUE ~ "NA/Unknown"))

data_F <- data %>% filter(RAD_TYPE_F != "No Radiation")
...
#no unknowns
known_data_F <- data_F %>% filter(RAD_TYPE_F != "Unknown")

p_table(known_data_F,
        vars = c("RAD_TYPE_F"))
...
known_data_F %>%
    ggplot(aes(x = YEAR_OF_DIAGNOSIS)) + 
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    geom_histogram(stat = "count", aes (fill = RAD_TYPE_F), position = "fill") +
  labs(x = "Year of Diagnosis", 
       y = "Proportion of Total Patients (%)",
       title = "Type of Radiation Therapy Used By Year",
       fill="Radiation Type") + 
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))

# is the decline in EBRT significant?
prop.test(c(243, 161),c(350,266))


prop.test(c(105, 105),c(350,266))
```


##Model for radiation as treatment
```{r}

fit_rad <- data %>%
        filter(T_SIZE != "Microscopic focus" & T_SIZE != "No Tumor" &
                 T_SIZE != "NA_unk") %>%
        filter(RADIATION_YN != "Ukn") %>%
        mutate(FACILITY_TYPE_F = fct_relevel(FACILITY_TYPE_F, 
                                             "Academic/Research Program", 
                                             after = 0)) %>%
          mutate(T_SIZE = fct_relevel(T_SIZE, 
                                             "1-2 cm", 
                                             after = 0)) %>%
        mutate(RADIATION_YN = case_when(RADIATION_YN == "Yes" ~ TRUE,
                                        RADIATION_YN == "No" ~ FALSE)) %>%
        glm(RADIATION_YN ~ FACILITY_TYPE_F + AGE_F + RACE_F + SEX_F + 
                    CDCC_TOTAL + T_SIZE + YEAR_OF_DIAGNOSIS,
               family = "binomial",
            data = .)
...
exp(cbind("Odds ratio" = coef(fit_rad), confint.default(fit_rad, level = 0.95)))
```
...
<!-- ```{r, echo=FALSE, warning=FALSE, message=FALSE} -->
<!-- or <- as.data.frame(exp(coefficients(fit_rad))) -->
<!-- or$Variable <- rownames(or) -->
<!-- rownames(or) <- c() -->
<!-- names(or) <- c('OddsRatio', 'Variable') -->

<!-- ci <- as.data.frame(exp(confint(fit_rad))) -->
<!-- ci$Variable <- rownames(ci) -->
<!--   # rownames(ci) <- c() -->

<!-- all <- full_join(or, ci, by = 'Variable') -->
<!-- names(all) <- c('OddsRatio', 'Variable', 'Lower', 'Upper') -->
<!-- all <- na.omit(all) -->

<!-- all <- all %>% -->
<!--         filter(Variable != '(Intercept)') %>% -->
<!--         select(Variable, Lower, OddsRatio, Upper) %>% -->
<!--         mutate(Sig = ifelse(Lower < 1 & Upper > 1, FALSE, TRUE))  -->

<!-- kable(all) -->

<!-- all %>% ggplot(aes(x = OddsRatio, y = Variable)) +  -->
<!--         geom_point(shape = 18, size = 2) +  -->
<!--         geom_errorbarh(aes(xmin = Lower, xmax = Upper), height = 0.1) +  -->
<!--         geom_vline(xintercept = 1, linetype = 'dashed') +  -->
<!--         ggtitle('Odds Ratios with 95% CI') + -->
<!--         ylab('') + xlab("Odds Ratio (log)") + -->
<!--         theme_bw() + -->
<!--         scale_x_log10() -->
<!-- ``` -->
...
<!--                                         RADIATION_YN == "No" ~ FALSE)) %>% -->
<!--         glm(RADIATION_YN ~ FACILITY_TYPE_F + AGE_F + RACE_F + SEX_F +  -->
<!--                     CDCC_TOTAL + T_SIZE + YEAR_OF_DIAGNOSIS + HISTOLOGY, -->
<!--                family = "binomial", -->
<!--             data = .) -->
...
<!-- exp(cbind("Odds ratio" = coef(fit_rad2), confint.default(fit_rad2, level = 0.95))) -->
<!-- ``` -->
...
# Exclude radiation types with <5 instances
data <- data %>% filter(RAD_TYPE != "Neutron +/- Photons/Electrons" 
                               & RAD_TYPE != "Electrons" 
                               & RAD_TYPE != "Brachytherapy, Interstitial, LDR"
                               & RAD_TYPE != "Cobalt-60, Cesium-137"
                               & RAD_TYPE != "Radioisotopes, NOS"
                               & RAD_TYPE != "Photons (2-5 MV)"
                               & RAD_TYPE != "Other, NOS")

# plot crowflys (how far do people travel for each type of radiation?)
ggplot(data = data) + 
  geom_histogram(mapping = aes(x = CROWFLY)) + 
  facet_wrap(~RAD_TYPE) + 
  xlim(0, 300)

# plot violin plots of crowflys (compare distances traveled for radiation types)
ggplot(data = data) + 
  geom_violin(mapping = aes(x = RAD_TYPE, y = CROWFLY)) + 
  ylim(0, 250)

p_table(data,
        vars = c("CROWFLY", "FACILITY_LOCATION_F"),
        strata = c("RAD_TYPE"))

p_table(data,
        vars = c("RAD_TYPE", "CROWFLY"),
...
# checking if income bracket/insurance status affects radiation utilization
...
p_table(data,
        vars = c("RADIATION_YN"),
        strata = c("INSURANCE_F"))

data %>%
    ggplot(aes(x = INSURANCE_F)) + 
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    geom_histogram(stat = "count", aes (fill = RADIATION_YN), position = "fill") 
```
...
# what predicts getting any kind of treatment vs nothing
...
fit_tx <- data %>%
        ...
        mutate(RADIATION_YN = case_when(RADIATION_YN == "Yes" ~ TRUE,
                                        RADIATION_YN == "No" ~ FALSE)) %>%
        glm((SURG_RAD_SEQ != "No Treatement") ~ FACILITY_TYPE_F + AGE_F + RACE_F + SEX_F + 
                    CDCC_TOTAL + T_SIZE + YEAR_OF_DIAGNOSIS,
               family = "binomial",
            data = .)
...
```{r}
# tx_fit <- survfit(Surv(DX_LASTCONTACT_DEATH_MONTHS, PUF_VITAL_STATUS == 0)
#                       ~ SURG_RAD_SEQ,
#                   data = data)
# # 
# # #Plot for paper
...
#            linetype = "strata",
#            xlab = "Time (Months)",
#            legend.labs = c("Surgery Alone", "Surgery then Radiation", "Radiation Alone", "No Treatment", "Other"),
...
model_one <- coxph(Surv(DX_LASTCONTACT_DEATH_MONTHS, PUF_VITAL_STATUS == 0)
                     ~ SURGERY_YN + AGE + SEX_F + RACE_F + CDCC_TOTAL + T_SIZE,
                     data = data)
model_one %>% summary()
...
# comparing time from diagnosis to radiation
```{r}
ggplot(data, mapping = aes(x = YEAR_OF_DIAGNOSIS, y = DX_RAD_STARTED_DAYS)) + 
  geom_violin()

p_table(data,
        vars = c("DX_RAD_STARTED_DAYS"),
        strata = c("YEAR_OF_DIAGNOSIS"))

t.test((data %>% filter(YEAR_OF_DIAGNOSIS == 2004))$DX_RAD_STARTED_DAYS, 
       (data %>% filter(YEAR_OF_DIAGNOSIS == 2014))$DX_RAD_STARTED_DAYS)
```
